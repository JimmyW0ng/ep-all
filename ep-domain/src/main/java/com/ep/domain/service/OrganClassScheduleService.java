...
public class OrganClassScheduleService {
     ...
     * @return
     */
    public ResultDo<Page<OrganClassCatalogCommentBo>> getClassCatalogChildren(Pageable pageable,
                                                                              EpOrganAccountPo organAccountPo,
                                                                              Long classId,
                                                                              Long startTimeStamp,
                                                                              String nickName) {
        log.info("获取课时评价成员列表, page={}, accountId={}, classId={}, startTimeStamp={}", pageable, organAccountPo.getId(), classId, startTimeStamp);
        ResultDo<Page<OrganClassCatalogCommentBo>> resultDo = ResultDo.build();
        // 校验课程
        ...
        Page<OrganClassCatalogCommentBo> data = organClassScheduleRepository.findChildrenByClassIdAndStartTime(pageable, classId, startTime, nickName);
        List<OrganClassCatalogCommentBo> children = data.getContent();
        if (CollectionsTools.isNotEmpty(children)) {
            for (OrganClassCatalogCommentBo bo : children) {
                // 头像
     ...
     * @return
     */
    public ResultDo<OrganClassCatalogCommentDto> getClassCatalogCommentView(EpOrganAccountPo organAccountPo, Long classScheduleId) {
        log.info("课时评价初始化, accountId={}, classScheduleId={}", organAccountPo.getId(), classScheduleId);
        ...
        // 班次行程信息
        Optional<OrganClassCatalogCommentBo> existSchedule = organClassScheduleRepository.findCatalogCommentById(classScheduleId);
        if (!existSchedule.isPresent()) {
            log.error("班次行程信息不存在, classId={}, classScheduleId={}", organAccountPo.getId(), classScheduleId);
            return resultDo.setError(MessageCode.ERROR_CLASS_SCHEDULE_NOT_EXIST);
        }
        OrganClassCatalogCommentBo bo = existSchedule.get();
        // 头像
        Optional<EpFilePo> optional = fileRepository.getOneByBizTypeAndSourceId(BizConstant.FILE_BIZ_TYPE_CODE_CHILD_AVATAR, bo.getChildId());
        String avatar = optional.isPresent() ? optional.get().getFileUrl() : null;
        bo.setAvatar(avatar);
        if (bo.getEvaluateFlag()) {
            // 加载评论
            Optional<EpMemberChildCommentPo> existComment = memberChildCommentRepository.findDetailByChildIdAndClassScheduleId(bo.getChildId(), bo.getClassScheduleId());
            if (existComment.isPresent()) {
                bo.setComment(existComment.get().getContent());
            }
            // 加载标签
        ...
        OrganClassCatalogCommentDto commentDto = new OrganClassCatalogCommentDto(bo, courseTagList);
    ...
    public void batchInitByClassId(Long classId, List<EpOrderPo> openingOrders) {
        ...
        for (EpOrderPo orderPo : openingOrders) {
            ...
            for (EpOrganClassCatalogPo catalogPo : catalogPos) {
                ...
                schedulePo.setDuraType(EpOrganClassScheduleDuraType.valueOf(catalogPo.getDuraType().getLiteral()));
